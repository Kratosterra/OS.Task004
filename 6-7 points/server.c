...
int main(int argc, char *argv[])
{
    ...
    struct sockaddr_in address, client_addr, observer;
    int opt = 1;
    ...
    printf("Агроном ждёт наблюдателя...");
    if (bind(server_fd, (struct sockaddr *)&address, sizeof(address)) < 0)
    {
        perror("Ошибка при настройке сокета.\n");
        exit(EXIT_FAILURE);
    }
    recvfrom(server_fd, NULL, 0, 0, (struct sockaddr *)&observer, &addrlen);
    printf("Наблюдатель на месте.");
    sendto(server_fd, &a, sizeof(double), 0, (struct sockaddr *)&observer, sizeof(observer));
    sendto(server_fd, &b, sizeof(double), 0, (struct sockaddr *)&observer, sizeof(observer));
    sendto(server_fd, &num_clients, sizeof(int), 0, (struct sockaddr *)&observer, sizeof(observer));
    ...
    while (num_clients > 0)
    {
        ...
        // Наблюдателю.
        sendto(server_fd, &client_subrange_start, sizeof(double), 0, (struct sockaddr *)&observer, sizeof(observer));
        sendto(server_fd, &client_subrange_end, sizeof(double), 0, (struct sockaddr *)&observer, sizeof(observer));
        sendto(server_fd, &num_clients, sizeof(int), 0, (struct sockaddr *)&observer, sizeof(observer));
        ...
        // Наблюдатель
        sendto(server_fd, &client_result, sizeof(double), 0, (struct sockaddr *)&observer, sizeof(observer));
        sendto(server_fd, &total_area, sizeof(double), 0, (struct sockaddr *)&observer, sizeof(observer));
